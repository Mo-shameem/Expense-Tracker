  ...
  return (
    <HashRouter>
      ...
      {!user ? (
        ...
        <Layout user={user} onLogout={logout} pendingCount={pendingCount} onManageCategories={() => setShowCategories(true)}>
          <Routes>
            <Route path="/" element={<CalendarPage {...sharedProps} />} />
            <Route path="/charts" element={
              <Suspense fallback={<div className="p-4 space-y-3"><Skeleton className="h-10 rounded-2xl" /><Skeleton className="h-64 rounded-2xl" /></div>}>
          ...
          {showCategories && (
            <CategoriesSheet
              ...
              onAdd={async (cat) => {
                try { await addCategory(cat); showToast('Category added', 'success') }
